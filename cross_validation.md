...
\##若有一个清晰的hypothesis，则会简单很多，just test
it，但很多时候你都没有一个清晰的hypothesis，你需要去做model selection

\##model太简单，没有包括全部的重要变量；too
complex，包括很多不重要的变量 \##underfitting;overfitting \##high
bias;high variance \##too simple-underfitting-high bias \##too
complex-overfitting-high variance

\##根据现有dataset构建model，用一个新的dataset去测试model是否足够准确，但是现实中很难获得一个新的dataset—
cross validation（training set；test set）–选一个root mean squared
error最小的model（`rmse()`直接帮你计算）
...
\##`mgcv` package是用于各种nonlinear model，而`gam`是其中用来fitting a
generalized additive model,其中`s()`代表的是do somthing smooth
...
\##让smooth有更多波动，`k`–sort of polynomials to use, `sp`–smoothing
parameter,越小就代表最后的线越不smooth
...
\##`rmse(model, dataset)` \##选rmse最小的model– smooth model
...
\##根据以上rmse的比较，从数值上看smooth似乎并没有比wiggly强很多，但从图片上看能看出很大区别，所以we
just do it over and over
again,看一个平均的差异，而不是单个test结果的差异

## ITERATE!!

\##可以重复上面的步骤，先sample_frac,再anti_joint,不过我们有更聪明的方法–直接使用`crossv_mc(dataset, 操作次数)`–把dataset按照默认的分法（80/20）randomly把数据分成training
set 和 test
set，这样的操作重复n次，但分开的data是一个resample的形式，并非dataframe，所以之后需要对数据进行`as.tibble`的操作
...
\##导出training的数据，查看第3次split的时候training
set具体是由哪些数据组成的
...
\##若只有一个argument在function中，则可以直接使用anonymous
function,不需要单独写出function
...
\##piece linear model–model分成了不同的部分，每个部分都是linear
model–weight_cp7，在weight=7的地方做一次分割，（从图片看出来是在weight=7有一个转折）
