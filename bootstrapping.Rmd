...
sim_df_nonconst |> 
  ...
  knitr::kable(digits = 3)
```
可以看到这两个样本的sd都是很相近的（方差不相等的结果和方差相等的结果一样），但其实并不是这样，这里sim_df_nonconst得出的结果并不是真实的结果，`lm`是按照assumption得出的数据，但是其实sim_df_nonconst并不符合这些assumption，但我们可以通过bootstrapping模拟得出它在真实data generating mechanism下产生的结果。
...
`modelr` package中有`bootstrap`这个function可以直接用，不用像上面那样自己写公式--`bootstrap(dataset，bootstrap的次数)`
```{r}
bootstrap_results = 
  sim_df_const |> 
  bootstrap(n = 5000) |> 
